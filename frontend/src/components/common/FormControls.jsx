...
const controlBase =
  'block w-full rounded-lg border bg-white px-3 py-2 text-sm text-slate-900 shadow-sm transition-colors placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500/30 disabled:bg-slate-50 disabled:text-slate-500'

export const Input = forwardRef(function Input({ className, error, ...props }, ref) {
  return (
    <input
      ref={ref}
      className={cn(controlBase, error ? 'border-red-300 focus:border-red-400' : 'border-slate-300 focus:border-brand-500', className)}
      {...props}
    />
  )
})
...
export function Checkbox({ className, label, ...props }) {
  return (
    <label className={cn('inline-flex select-none items-center gap-2 text-sm text-slate-700', className)}>
      <input
        type="checkbox"
